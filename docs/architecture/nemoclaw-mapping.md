...
**NemoClaw** is a conceptual extension of the OpenClaw agent platform architecture designed for coordinating multiple specialized agents working together on complex, multi-step workflows. While OpenClawNet currently implements a single-agent orchestrator, this document reserves the space and architectural patterns needed to evolve toward NemoClaw-style multi-agent coordination.
...
| Single `IModelClient` | Multiple models, one per agent type | Domain-specific model optimization |
...
- SignalR can stream workflow progress to UI
